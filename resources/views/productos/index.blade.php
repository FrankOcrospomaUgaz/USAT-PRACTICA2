  ...
  <div class="card-header d-flex align-items-center justify-content-between flex-wrap">
    <div class="flex-grow-1">
      <form class="form-inline">
        <input name="q" value="{{ $q }}" class="form-control form-control-sm mr-2" placeholder="Buscar producto...">
      ...
      <tbody>
        @forelse($productos as $p)
          <tr>
            ...
            <td>{{ $p->stock }}</td>
            <td class="text-right">
              <button class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#modalGallery{{ $p->id }}">
                <i class="fas fa-images"></i>
              </button>
              <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#modalEdit{{ $p->id }}">
                <i class="fas fa-edit"></i>
              </button>
              <form class="d-inline form-eliminar" method="POST" action="{{ route('productos.destroy', $p) }}">
                @csrf @method('DELETE')
        ...
        @empty
          <tr><td colspan="7" class="text-center text-muted py-3">No hay productos registrados</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
      ...
      <div class="modal-body">
        <div class="form-row">
          ...
          <div class="form-group col-md-6">
            ...
            <select id="categoriaCreate" name="categoria_id" class="form-control form-control-sm" required></select>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="number" step="0.01" min="0" name="precio" class="form-control form-control-sm" required>
          </div>
          <div class="form-group col-md-6">
            <label>Stock</label>
        ...
        <div class="form-group">
          <label>Galería de imágenes (puede seleccionar varias)</label>
          {{-- ✅ FIX: input múltiple funcional --}}
          <input type="file" name="galeria[]" id="galeriaCreate" class="form-control-file" accept="image/*" multiple>
        </div>
      </div>
...
@foreach($productos as $p)
  ...
  <div class="modal fade" id="modalEdit{{ $p->id }}">
    <div class="modal-dialog modal-lg">
      <form class="modal-content" method="POST" action="{{ route('productos.update', $p) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="modal-body">
          <div class="form-row">
            ...
            <div class="form-group col-md-6">
              <label>Categoría</label>
              <select id="catEdit{{ $p->id }}" name="categoria_id" class="form-control form-control-sm" required>
          ...
          <div class="form-group">
            <label>Descripción</label>
            <textarea name="descripcion" class="form-control form-control-sm" rows="2">{{ $p->descripcion }}</textarea>
          </div>
          ...
          <div class="form-group">
            <label>Agregar imágenes a galería (puede seleccionar varias)</label>
            {{-- ✅ FIX: asegurar atributo multiple --}}
            <input type="file" name="galeria[]" id="galeriaEdit{{ $p->id }}" class="form-control-file" accept="image/*" multiple>
          </div>
        </div>
        <div class="modal-footer">
